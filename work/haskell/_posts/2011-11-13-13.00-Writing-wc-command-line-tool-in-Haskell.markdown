...
about the way **IO** is handled. In **Haskell** as mentioned early in my posts is a
purely functional language and for things such as **IO** which are basically
side effects within your function **Haskell**. This side-effect is handled by 
expressing side effects as a **Monad**. Now I won't go into all of the details
of a **Monad** in this post and I suggest you read up on Monads with these 
few links:
...
entry point. We had to use the **show** function to convert the number calculated
by our countlines function back into a **String**. With the above you should be 
able to run commands such as:
